	...
	return (
        <div style={{ border: '2px solid black', padding: '20px' }}>
            <div
                style={{
                    display: 'grid',
                    gridTemplateColumns: cities.length % 2 === 0 ? '1fr 1fr' : '1fr 1fr',
                    gridAutoRows: 'auto',
                    gap: '10px'
                }}>
                {cities.map((city, index) => (
                    <div
                        ...
                        style={{
                            gridColumn: cities.length % 2 !== 0 && index === cities.length - 1 ? 'span 2' : 'span 1', // Full width for the last item if odd
                            border: '1px solid gray',
                            padding: '10px'
                        }}>
                        <h1>
                            {city.city} {city.isCapital ? '(capital)' : ''}
                        </h1>
                        <p>
                            {city.city} city is located in {city.location.continent}, {city.location.country} and has a population of{' '}
                            {city.population} people.
                        </p>
                        {city.isCapital && (
                            <p>
                                {city.city} is the capital of {city.location.country}.
                            </p>
                        )}
                        ...
                        {city.touristAttractions.length === 1 ? (
                            <p>
                                Main Tourist attraction of {city.city} is {city.touristAttractions[0]}.
                            </p>
                        ) : city.touristAttractions.length > 1 ? (
                            <p>
                                Main Tourist attractions of {city.city} are {city.touristAttractions.join(', ')}.
                            </p>
                        ) : null}
                    </div>
                ))}
